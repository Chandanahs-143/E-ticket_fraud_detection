...
def ml_predict_single(amount,tickets,device,location,hour):
    if not MODEL_LOADED: return _fallback(amount,tickets,device,location,hour)
    ...
    return int(round(model.predict_proba(X)[0][1]*100))
...
def _fallback(amount,tickets,device,location,hour):
    if hour < 6 or hour > 22:
        s += 15
    if amount>7000:s+=30
    elif amount>4000:s+=15
    elif amount>2000:s+=8
    if tickets>10:s+=25
    elif tickets>5:s+=12
    s+={"Mobile":5,"Laptop":10,"Tablet":8}.get(device,10)
    s+={"Bangalore":5,"Delhi":12,"Mumbai":8,"Hyderabad":7,"Chennai":6,"Pune":9}.get(location,10)
